...
- **Schema.org Accessibility Metadata (JSON-LD)** - Added machine-readable accessibility declaration using W3C approved vocabulary. Formally declares WCAG 2.2 AA+ compliance, semantic landmarks, keyboard/mouse/touch controls, no accessibility hazards, and support for textual-only access modes. Impact: +1.2 KB (+2%), ~24ms on 3G, non-blocking. Benefits: SEO enhancement, assistive technology discovery, transparent compliance documentation. Aligns with all 4 priorities. Full details in docs/decisions.md.
- **Colorful Navigation Underlines** - Replaced static white underlines with vibrant brand colors: turquoise (#00FFFF) for Services, purple (#FF00FF) for Industries, branded orange (#ff6a13) for Insights, green (#00FF00) for Contact. Pure CSS animated underlines (0 → 100%, 0.3s ease) applied to header and footer navigation. Removed hover background for cleaner effect. Progressive enhancement: works on IE8+, animates on IE10+. Zero performance impact. Full color palette documentation in docs/decisions.md.
- **Branded Footer Logo** - Replaced text-based footer logo with white branded image (200px width). Added aspect-ratio: 620/69 for CLS prevention, responsive with max-width: 100%. Maintains all semantic structure (h2 wrapper) and spacing. Consistent brand identity across header and footer. Zero CLS impact. Full details in docs/decisions.md.
- **Centered Statistics Cards** - Added text-align: center to statistics cards (4.9/5, 24h, 100%) for improved visual balance and readability. Single CSS property change, universal improvement across all screen sizes.
